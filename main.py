...
def phase_prepare(script_text: str, config: Config, video_name: str = "video", auto_continue: bool = False):
    ...
    if all_covered:
        print("\n🎉 All segments covered from library! No images needed.")
        if auto_continue:
        ...
        print(f"\n📋 Shopping list saved: {config.shopping_list_path}")
        print(f"   Download images → {config.input_dir}/")
        print(f"   Then run: python main.py continue\n")
...
def phase_continue(config: Config):
    ...
    if still_missing:
        print(f"\n⚠️  {len(still_missing)} segments still missing images:")
        for a in still_missing:
            seg = segments[a.segment_index]
            print(f"   ❌ Seg {a.segment_index + 1:02d}: \"{seg.text[:60]}...\"")
            print(f"      Expected: {config.input_dir}/{a.segment_index + 1:02d}.jpg")
    ...
    print(f"\n✅ Video generated: {final_output}")
    print(f"   Library now has images from {sum(1 for a in assignments if a.source == 'user_input')} new + existing entries.\n")
